export async function onRequestPost({ request, env, params }) {
  ...
  const stage = form.get("stage") || "unspecified";
  ...
  const key = `workorders/${params.id}/${Date.now()}.${ext}`;
  ...
  await env.DB.prepare(
    "INSERT INTO photos (entity_type, entity_id, stage, r2_key) VALUES ('work_order', ?, ?, ?)"
