...
export async function onRequestPost({ request, env }) {
  ...
  // Deliberately generic error for both "no such user" and "wrong PIN" — don't
  // reveal which one, so someone probing usernames can't tell which exist.
  ...
  const isValid = user.pin_hash && user.pin_salt
    ? await verifyPin(pin, user.pin_salt, user.pin_hash)
    : false;
  ...
  // Success — clear lockout state and issue a session token.
  await env.DB.prepare(
  ...
  const token = await signToken(
    { id: user.id, name: user.name, role: user.role, resellerName: user.reseller_name, tokenVersion: user.token_version, exp },
    secret
  );

  return Response.json({ token, name: user.name, role: user.role });
}
