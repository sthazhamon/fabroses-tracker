...
export async function onRequestPatch({ request, env, params }) {
  ...
  if (action === "revoke_sessions") {
    // Bumping token_version instantly invalidates every existing session for
    // this user, even ones that haven't hit their 12-hour expiry yet.
  ...
  if (action === "reset_pin") {
    ...
    await env.DB.prepare(
      `UPDATE users SET pin_hash = ?, pin_salt = ?, failed_attempts = 0, locked_until = NULL, token_version = token_version + 1
       WHERE id = ?`
