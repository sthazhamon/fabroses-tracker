export async function onRequestPost({ request, env, params }) {
  ...
  if (order.status !== "billed") {
    return Response.json({ error: "Bill this order before shipping it" }, { status: 400 });
  }

  await env.DB.prepare(
    `UPDATE customer_orders SET status = 'shipped', courier = ?, tracking_id = ?, dispatch_date = ?, updated_at = datetime('now')
     WHERE id = ?`
